...
(async () => {
    ...
    try {
        ...
        registryNodes[0].state='standby'; registryNodes[0].status='online';
        await page.evaluate(() => loadStatus());
        const standbyCard = page.locator('#component-grid article').filter({hasText:'Standby world'});
        assert.equal(await standbyCard.getByRole('button',{name:'Start',exact:true}).isEnabled(),false,'standby already has a process');
        assert.equal(await standbyCard.getByRole('button',{name:'Stop',exact:true}).isEnabled(),true,'standby can stop without a world command');
        assert.equal(await promote.isEnabled(),true,'prepared standby can be promoted without relaunch');
        assert.match(await standbyCard.innerText(),/Static data loaded/);
        registryNodes[0].state='unresponsive'; await page.evaluate(() => loadStatus());
        assert.equal(await promote.isEnabled(),false,'stale standby cannot be promoted');
        registryNodes[0].state='standby'; await page.evaluate(() => loadStatus());
