...
(async()=>{
 ...
 try {
  ...
  assert.match(await page.locator('.metric-panel').filter({hasText:'Process memory'}).textContent(),/Metrics unavailable/);
  await page.evaluate(()=>dashboard.stale());
  assert.match(await page.locator('.metric-panel').filter({hasText:'Process memory'}).textContent(),/Stale/);
  ...
  await page.evaluate(()=>{fixture.services.push({key:'characters',name:'Characters',characterserver:true,state:'running',metricsAvailable:true,cpu:3,memoryMiB:70,connections:2,pendingRequests:1,latencyMs:1.5,failures:4,uptimeSeconds:60});dashboard.update(fixture,1020000);});
  await page.getByLabel('Server for metric graphs').selectOption('characters');
  assert.equal(await page.locator('.metric-panel:visible').count(),6);
  assert.match(await page.locator('.metric-panel').filter({hasText:'Character connections'}).textContent(),/2 connections/);
  await page.evaluate(()=>{fixture.services[2].metricsAvailable=false;dashboard.update(fixture,1025000);});
  assert.match(await page.locator('.metric-panel').filter({hasText:'Character connections'}).textContent(),/Metrics unavailable/);
