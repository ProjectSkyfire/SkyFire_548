            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_FLAME_SHOCK:
                        ...
                        events.ScheduleEvent(EVENT_FLAME_SHOCK, std::rand() % 15000 + 10000);
                        break;
                    case EVENT_ARCANE_SHOCK:
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                            DoCast(target, SPELL_ARCANE_SHOCK);
                        events.ScheduleEvent(EVENT_ARCANE_SHOCK, std::rand() % 15000 + 10000);
                        break;
                    case EVENT_FROST_SHOCK:
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                            DoCast(target, SPELL_FROST_SHOCK);
                        events.ScheduleEvent(EVENT_FROST_SHOCK, std::rand() % 15000 + 10000);
                        break;
                    case EVENT_SHADOW_SHOCK:
                        if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                            DoCast(target, SPELL_SHADOW_SHOCK);
                        events.ScheduleEvent(EVENT_SHADOW_SHOCK, std::rand() % 15000 + 10000);
                        break;
