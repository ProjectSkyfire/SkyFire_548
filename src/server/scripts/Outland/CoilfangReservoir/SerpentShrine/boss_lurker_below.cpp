        ...
        void UpdateAI(uint32 diff) OVERRIDE
        {
            ...
            else // submerged
            {
                ...
                if (!Spawned)
                {
                    ...
                    // spawn adds
                    for (uint8 i = 0; i < 9; ++i)
                        if (Creature* summoned = me->SummonCreature(i < 6 ? NPC_COILFANG_AMBUSHER : NPC_COILFANG_GUARDIAN, AddPos[i][0], AddPos[i][1], AddPos[i][2], 0, TempSummonType::TEMPSUMMON_CORPSE_DESPAWN, 0))
