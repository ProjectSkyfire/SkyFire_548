...
public:
    ...
    {
        if (lang != Language::LANG_ADDON && sWorld->getBoolConfig(CONFIG_CHATLOG_WHISPER))
            SF_LOG_DEBUG("chat.log", "[WHISPER] Player %s tells %s: %s",
                player->GetName().c_str(), receiver ? receiver->GetName().c_str() : "<unknown>", msg.c_str());
        else if (lang == Language::LANG_ADDON && sWorld->getBoolConfig(CONFIG_CHATLOG_ADDON))
    ...
    void OnChat(Player* player, ChatMsg type, Language lang, std::string& msg, Group* group)
    {
        ...
        switch (type)
        {
            case ChatMsg::CHAT_MSG_PARTY:
                if (lang != Language::LANG_ADDON && sWorld->getBoolConfig(CONFIG_CHATLOG_PARTY))
            ...
            case ChatMsg::CHAT_MSG_RAID:
                if (lang != Language::LANG_ADDON && sWorld->getBoolConfig(CONFIG_CHATLOG_RAID))
            ...
            case ChatMsg::CHAT_MSG_INSTANCE:
                if (lang != Language::LANG_ADDON && sWorld->getBoolConfig(CONFIG_CHATLOG_BGROUND))
    ...
    void OnChat(Player* player, ChatMsg type, Language lang, std::string& msg, Guild* guild)
    {
        switch (type)
        {
            case ChatMsg::CHAT_MSG_GUILD:
                if (lang != Language::LANG_ADDON && sWorld->getBoolConfig(CONFIG_CHATLOG_GUILD))
