...
public:
    ...
    struct boss_protector_kaolanAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            if (instance)
            {
                if (Creature* regail = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_REGAIL)))
                    if (regail->isDead())
                        regail->Respawn();

                if (Creature* asani = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_ASANI)))
                    if (asani->isDead())
                        asani->Respawn();
...
public:
    ...
    struct boss_elder_regailAI : public ScriptedAI
    {
        ...
        void Reset() override
        {
            ...
            if (instance)
            {
                if (Creature* kaolan = ObjectAccessor::GetCreature(*me, instance->GetData64(DATA_KAOLAN)))
                    if (kaolan->isDead())
                        kaolan->Respawn();
