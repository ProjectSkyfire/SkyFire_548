...
enum eShadowOfDoubtSpells //World Boss - Sha de la duda / Simple script
{
    SPELL_DEAFENED        = 108918,
    SPELL_SHA_BOLT        = 126139,
    SPELL_GROWING_DOUBT   = 126147,
	SPELL_WITHER          = 106736,
	SPELL_NOTHING         = 106113,
};
...
{
    public:
        mob_shadow_of_doubt() : CreatureScript("mob_shadow_of_doubt")//scriipt
        {
        }
        ...
        struct mob_shadow_of_doubtAI : public ScriptedAI
        {
            ...
            void Reset()
            {
				...
				events.ScheduleEvent(EVENT_WITHER,        11000);
				events.ScheduleEvent(EVENT_NOTHING,       10000);
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
						...
						case EVENT_WITHER:
						    if (Unit* target = SelectTarget(SELECT_TARGET_TOPAGGRO))
							    me->CastSpell(target, SPELL_WITHER, false);
							events.ScheduleEvent(EVENT_WITHER, 11000);
							break;
						case EVENT_NOTHING:
						    if (Unit* target = SelectTarget(SELECT_TARGET_TOPAGGRO))
							   me->CastSpell(target, SPELL_NOTHING, false);
							events.ScheduleEvent(EVENT_NOTHING, 10000);
							break;
                        default:
                            break;
    ...
    public:
        ...
        struct mob_kor_nas_nightsavageAI : public ScriptedAI
        {
            ...
            void Reset()
            {
				...
				events.ScheduleEvent(EVENT_TETANUS,  16000);
            }
                    ...
                    switch (eventId)
                    {
						...
						case EVENT_TETANUS:
						    if (Unit* target = SelectTarget(SELECT_TARGET_TOPAGGRO))
							   ...
							   events.ScheduleEvent(EVENT_TETANUS, 35000);
							   break;
                        default:
                            break;
                    }
