    ...
    public :
        ...
        void Update(uint32 uiDiff)
        {
            ...
            if((GetData(INSTANCE_DATA_OOK_OOK_STATUS) == DONE) && (GetData(INSTANCE_DATA_HOPTALLUS_STATUS) == NOT_STARTED) && !m_bHoptallusActive)
            {
                ...
                {
                    uint32 entry = RAND<uint32>(56718, 59426, 56631);
        ...
        void DoStartOokOok()
        {
            if(Creature* ookOok = instance->GetCreature(m_uiOokOokGuid))
                ookOok->AI()->DoAction(0);
        }

        void DoStartHoptallus()
        {
            if(Creature* hoptallus = instance->GetCreature(m_uiHoptallusGuid))
