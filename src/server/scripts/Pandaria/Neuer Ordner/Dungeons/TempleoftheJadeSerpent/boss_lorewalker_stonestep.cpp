    ...
    public:
        ...
        struct boss_lorewalker_stonestep_AI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                GetCreatureListWithAuraInGrid(creatures, me, SPELL_SHA_CORRUPTION, 1000.0f);
                GetCreatureListWithAuraInGrid(creatures, me, SPELL_SHA_CORRUPTION_3, 1000.0f);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_INTRO_3:
                        {
                            ...
                            if (Creature* scroll = me->GetMap()->GetCreature(me->GetInstanceScript()->GetData64(CREATURE_SCROLL)))
                        ...
                        {
                            if(statusBattle >= STATUS_LOREWALKER_STONESTEP_INTRO && statusBattle < STATUS_LOREWALKER_STONESTEP_FINISH && instance->IsWipe())
                                Reset();
                            else
                                events.ScheduleEvent(EVENT_WYPE_CHECKER, 3000);
                            break;
    ...
    struct mob_lorewalker_scroll_AI : public ScriptedAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            for (std::list<Creature*>::iterator iter = creatures.begin(); iter != creatures.end(); ++iter)
            {
                ...
                c->SetUInt64Value(UNIT_FIELD_CHANNEL_OBJECT, me->GetGUID());
                c->SetUInt32Value(UNIT_FIELD_CHANNEL_SPELL, SPELL_CORRUPTION_CHANNEL);
                c->DespawnOrUnsummon(2000);
            }

            if (eventChoosen == EVENT_LOREWALKER_STONESTEP_SUNS)
            {
                Creature* lorewalker = me->GetMap()->GetCreature(me->GetInstanceScript()->GetData64(CREATURE_LOREWALKTER_STONESTEP));
        ...
        struct mob_sun_AI : public ScriptedAI
        {
            ...
            void DoAction(int32 action)
            {
                switch (action)
                {
                    ...
                    {
                        me->RemoveFlag(UNIT_FIELD_NPC_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
                        break;
                    }
            ...
            void JustDied(Unit* killer)
            {
                ...
                if (Creature* loreWalker = me->GetMap()->GetCreature(me->GetInstanceScript()->GetData64(CREATURE_LOREWALKTER_STONESTEP)))
                    if(loreWalker->GetAI())
        ...
        struct mob_haunting_sha_AI : public ScriptedAI
        {
            ...
            void DoAction(int32 action)
            {
                ...
                if(action == ACTION_ATTACK_ZAO)
                {
                    ...
                    if(Creature* zao = me->GetMap()->GetCreature(me->GetInstanceScript()->GetData64(CREATURE_ZAO_SUNSEEKER)))
                    {
...
class mob_peril_and_strife : public CreatureScript
{
    ...
    struct mob_peril_and_strife_AI : public ScriptedAI
    {
        ...
        Creature* GetOther()
        {
            ...
            return me->GetMap()->GetCreature(me->GetInstanceScript()->GetData64(isPeril ? CREATURE_STRIFE: CREATURE_PERIL));
        }
