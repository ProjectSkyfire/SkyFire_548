                ...
                switch(uint32 eventId = events.ExecuteEvent())
                {
                    ...
                    case EVENT_NOVICE_DEFEATED:
                    {
                        ...
                        for (auto novice : noviceList)
                        {
                            ...
                            {
                                if (Creature* position = instance->instance->GetCreature(guid))
                                {
        ...
        struct npc_snowdrift_noviceAI : public ScriptedAI
        {
            ...
            void DamageTaken(Unit* attacker, uint32& damage)
            {
                ...
                if (damage >= me->GetHealth())
                {
                    ...
                    {
                        if (Creature* position = pInstance->instance->GetCreature(guid))
                        {
