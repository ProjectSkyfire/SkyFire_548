    ...
    public:
        ...
        struct boss_fengAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                {
                    if (pInstance)
                        if (Creature* phaseController = pInstance->instance->GetCreature(phaseControllerList[i]))
            ...
            void DamageTaken(Unit* attacker, uint32& damage)
            {
                ...
                if (nextPhasePct)
                {
                    if (me->HealthBelowPctDamaged(nextPhasePct, damage))
                    {
                        ...
                        if (Creature* phaseController = instance->instance->GetCreature(phaseControllerList[newPhase - 1]))
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                switch (events.ExecuteEvent())
                {
                    ...
                    case EVENT_SIPHONING_SHIELD_END:
                    {
                        if (pInstance)
                        {
                            if (Creature* shield = pInstance->instance->GetCreature(shieldGUID))
                            {
...
class mob_feng_shield : public CreatureScript
{
    public:
        ...
        struct mob_feng_shieldAI : public ScriptedAI
        {
            ...
            Creature* GetFeng()
            {
                return (pInstance) ? pInstance->instance->GetCreature(pInstance->GetData64(NPC_FENG)) : NULL;
            }
