            ...
            void UpdateAI(uint32 diff)
            {
                ...
                switch (events.ExecuteEvent())
                {
                    ...
                    case EVENT_TILE_DESACTIVATION:
                    {
                        ...
                        Creature* guardian = me->GetMap()->GetCreature(pInstance->GetData64(guardiansEntry[0]));

                        if (!guardian || !guardian->IsAlive()) // 0 can be the one despawned, we try another
                            guardian = me->GetMap()->GetCreature(pInstance->GetData64(guardiansEntry[1]));
                        ...
                        if (guardian) // They all have same life, we can get only get one
                        {
                            ...
                            {
                                for (uint32 entry: guardiansEntry)
                                    if (Creature* gardianForSpell = me->GetMap()->GetCreature(pInstance->GetData64(entry)))
...
class boss_generic_guardian : public CreatureScript
{
    public:
        ...
        struct boss_generic_guardianAI : public BossAI
        {
            ...
            {
                if (instance) return me->GetMap()->GetCreature(instance->GetData64(NPC_STONE_GUARD_CONTROLER)); else return NULL;
            }
            ...
            void Reset()
            {
                ...
                if (crystalGUID)
                    if (Creature* crystal = instance->instance->GetCreature(crystalGUID))
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                for (auto player: playerList)
                {
                    ...
                    if (spellToRemove)
                    {
                        ...
                        isActivated = true;
                        pInstance->DoAddAuraOnPlayers(SPELL_ENERGIZED_TILE_BUFF);
