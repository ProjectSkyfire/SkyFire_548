            ...
            void DoAction(int32 action)
            {
                ...
                switch (action)
                {
                    ...
                    case ACTION_SPIRIT_NEXT:
                    {
                        ...
                        if (Creature* spirit = pInstance->instance->GetCreature(pInstance->GetData64(spiritKingsEntry[lastActivated])))
                        {
    ...
    public:
        ...
        class spell_crazy_thought_SpellScript : public SpellScript
        {
            ...
            void HandleEffect(SpellEffIndex effIndex)
            {
                if (Unit* caster = GetCaster())
                {
                    uint32 chargesToAdd = GetSpellInfo()->Effects[EFFECT_0].BasePoints;
