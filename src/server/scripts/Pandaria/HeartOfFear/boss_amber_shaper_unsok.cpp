        ...
        struct boss_unsokAI : public BossAI
        {
            ...
            void JustDied(Unit* /*killer*/)
            {
                ...
                if (pInstance)
                {
                    ...
                    if (IsHeroic())
                        pInstance->DoCompleteAchievement(ACHIEVEMENT_HEROIC_UNSOK);
                }

                // Stopping amber pools
                std::list<Creature*> poolList;
                GetCreatureListWithEntryInGrid(poolList, me, NPC_AMBER_POOL_STALKER, 200.0f); // TODO: adjuste searche range
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_AMBER_EXPLOSION:
                        {
                            ...
                            GetPlayerListInGrid(mutatedList, me, 200.0f); // TODO: adjuste searche range
        ...
        struct mob_amber_scalpelAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                ...
                if (Creature* unsok = pInstance->instance->GetCreature(pInstance->GetData64(NPC_UNSOK)))
                {
                    ...
                    me->GetMotionMaster()->Clear();
                    if (Player* target = ObjectAccessor::FindPlayer(MAKE_NEW_GUID(targetGuid, 0, HighGuid::Player)))
        ...
        class spell_amber_scalpel_SpellScript : public SpellScript
        {
            ...
            void Summon()
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    GetCreatureListWithEntryInGrid(amberList, caster, NPC_LIVING_AMBER, 200.0f); // TODO: adjuste searche range
                    if (amberList.size() < 7)
                        if (urand(0, 1))
