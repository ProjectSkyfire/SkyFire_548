    ...
    public:
        ...
        struct boss_raigonnAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                if (Vehicle* meVehicle = me->GetVehicleKit())
                {
                    ...
                    if (Unit* passenger = meVehicle->GetPassenger(1)) // Check if weak_spot already spawned
                    {
                        ...
                        m_Instance->SendEncounterUnit(ENCOUNTER_FRAME_DISENGAGE, passenger);
                        return;
                    }
                    ...
                    if (Creature* weakSpot = me->SummonCreature(NPC_WEAK_SPOT, 0, 0, 0))
                    {
                        ...
                        if (m_Instance)
                            m_Instance->SetData64(NPC_WEAK_SPOT, weakSpot->GetGUID());
            ...
            void MoveInLineOfSight(Unit* who)
            {
                ...
                if (m_Instance)
                    if (m_Instance->GetBossState(DATA_RIMOK) != DONE)
                        return;
                ...
                m_Instance->SetBossState(DATA_RAIGONN, IN_PROGRESS);

                if (Creature* weakPoint = m_Instance->instance->GetCreature(m_Instance->GetData64(NPC_WEAK_SPOT)))
                {
            ...
            void DoEventCharge()
            {
                ...
                uint32 eventBrasierProgress = m_Instance->GetData(DATA_BRASIER_CLICKED);
                ...
                switch (eventChargeProgress)
                {
                    ...
                    case 2:
                        ...
                        if (m_Instance != nullptr)
                        {
                            if (!baseMovement)
                            {
                                if (GameObject* l_GreatDoor = GameObject::GetGameObject(*me, m_Instance->GetData64(GO_GREAT_DOOR)))
                                    l_GreatDoor->SendGameObjectActivateAnimKit(2213, false);
                            }
                        }
            ...
            bool checkStartBattle()
            {
                ...
                if (m_Instance)
                    if (m_Instance->GetData(DATA_RIMOK) != DONE)
                        return false;
...
class spell_getting_sun_battering_headbutt : public SpellScriptLoader
{
    public:
        spell_getting_sun_battering_headbutt() : SpellScriptLoader("spell_getting_sun_battering_headbutt") { }

        class spell_getting_sun_battering_headbutt_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_getting_sun_battering_headbutt_SpellScript);
            ...
            {
                OnObjectTargetSelect += SpellObjectTargetSelectFn(spell_getting_sun_battering_headbutt_SpellScript::FilterTarget, EFFECT_1, TARGET_UNIT_NEARBY_ENTRY);
                OnObjectTargetSelect += SpellObjectTargetSelectFn(spell_getting_sun_battering_headbutt_SpellScript::FilterTarget, EFFECT_2, TARGET_UNIT_NEARBY_ENTRY);
            }
        };

        SpellScript* GetSpellScript() const override
        {
            return new spell_getting_sun_battering_headbutt_SpellScript();
        }
};
...
void AddSC_boss_raigonn()
{
    ...
    /// Spells
    new spell_getting_sun_battering_headbutt();
}
