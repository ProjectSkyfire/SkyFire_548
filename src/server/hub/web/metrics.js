...
window.HubHealthDashboard = class {
    constructor(root) {
        ...
        this.summary = this.el('p', 'Waiting for server healthâ€¦', root); this.summary.setAttribute('role','status');
    ...
    reset() {
        ...
        this.lastSample = 0; this.lastHubUptime = null; this.latest = null; this.signature = ''; this.summary.textContent = 'Waiting for server healthâ€¦'; this.draw();
    }
    stale() {
        ...
        this.isStale = true; this.summary.textContent = 'Connection lost â€” health and graphs may be stale.'; this.draw(); }
    update(data, now = Date.now()) {
        ...
        this.summary.textContent = `Hub online Â· uptime ${Math.floor(data.hubUptime/3600)}h ${Math.floor(data.hubUptime%3600/60)}m Â· ${running}/${services.length} services running`;
    ...
    draw() {
        ...
        for (const chart of this.charts) {
            ...
            chart.current.textContent=typeof value==='number' && Number.isFinite(value) && value>=0 ? `${Number(value.toFixed(1))} ${chart.unit}` : this.isStale ? 'Stale â€” reconnecting' : 'Metrics unavailable';
            ...
            add('text',{x:38,y:153},'âˆ’10 min'); add('text',{x:317,y:153},'Now');
