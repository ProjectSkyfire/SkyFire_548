...
window.HubHealthDashboard = class {
    constructor(root) {
        ...
        for (const [key,title,unit] of [['players','Players online','players'],['cpu','CPU usage','%'],['update','World update time','ms'],['memoryMiB','Process memory','MiB'],['transfers','Active map transfers','transfers'],['traffic','Map data sent','MiB/s'],['connections','Service connections','connections'],['pendingRequests','Pending character requests','requests'],['latencyMs','Average character request latency','ms'],['failures','Service request failures','failures'],['requests','Chat service requests','requests'],['presencePlayers','Chat player presence','players'],['whisperRelays','Accepted whisper relays','relays'],['routedMessages','Routed chat messages','messages'],['routedRecipients','Selected chat recipients','recipients'],['routedControls','Admitted chat operations','operations']]) {
    ...
    draw() {
        ...
        for (const chart of this.charts) {
            const visible = service?.chatserver ? ['connections','requests','failures','presencePlayers','whisperRelays','routedMessages','routedRecipients','routedControls'] : service?.characterserver ? ['cpu','memoryMiB','connections','pendingRequests','latencyMs','failures'] :
