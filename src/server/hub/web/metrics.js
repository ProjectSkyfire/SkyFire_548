...
window.HubHealthDashboard = class {
    constructor(root) {
        this.root = root; this.history = new Map(); this.cards = new Map(); this.lastSample = 0; this.lastHubUptime = null;
        ...
        const label = this.el('label','Worldserver metrics',root); this.select = this.el('select','',label);
        this.select.setAttribute('aria-label','Worldserver for metric graphs'); this.select.addEventListener('change',()=>this.draw());
        ...
        for (const [key,title,unit] of [['players','Players online','players'],['cpu','CPU usage','%'],['update','World update time','ms']]) {
            ...
            this.charts.push({key,title,unit,current,svg});
        }
    }
    reset() {
        this.history.clear(); this.cards.clear(); this.health.replaceChildren(); this.select.replaceChildren();
    ...
    stale() {
        if (!this.isStale) for (const points of this.history.values()) {
            points.push({at: this.now || Date.now(), players: null, cpu: null, update: null});
    ...
    update(data, now = Date.now()) {
        ...
        if (this.lastHubUptime !== null && data.hubUptime < this.lastHubUptime) this.history.clear();
        this.lastHubUptime = data.hubUptime;
        const services = data.services;
        const running = services.filter(item=>item.state==='running').length;
        this.summary.textContent = `Hub online · uptime ${Math.floor(data.hubUptime/3600)}h ${Math.floor(data.hubUptime%3600/60)}m · ${running}/${services.length} managed services running`;
        for (const item of services) {
            ...
            card.children[2].textContent=`Uptime ${Math.floor((item.uptimeSeconds||0)/3600)}h ${Math.floor((item.uptimeSeconds||0)%3600/60)}m`;
        }
        for (const [key,card] of this.cards) if (!services.some(item=>item.key===key)) { card.remove(); this.cards.delete(key); this.history.delete(key); }
        const worlds=services.filter(item=>item.world);
        ...
        if (!this.lastSample || now-this.lastSample>=5000) {
            ...
            for (const world of worlds) {
                ...
                points.push({at:now,players:good?finite(world.players):null,cpu:good?finite(world.cpu):null,update:good?finite(world.update):null});
    ...
    draw() {
        ...
        for (const chart of this.charts) {
            ...
            const value=usable ? service[chart.key] : null;
            ...
            const valid=points.filter(point=>point[chart.key]!==null && point.at>=now-600000);
            ...
            for(const point of points) {
                if(point[chart.key]===null || point.at<now-600000) { previous=null; continue; }
