...
window.HubHealthDashboard = class {
    ...
    update(data, now = Date.now()) {
        ...
        for (const item of services) {
            ...
            if (item.chatserver && item.metricsAvailable) card.children[2].textContent+=` � Realms ${(item.chatRealms||[]).join(', ')} � ${item.connections} connections � ${item.requests} health probes � ${item.failures} errors`;
    ...
    draw() {
        ...
        for (const chart of this.charts) {
            const visible = service?.chatserver ? ['connections','requests','failures'] : service?.characterserver ? ['cpu','memoryMiB','connections','pendingRequests','latencyMs','failures'] :
