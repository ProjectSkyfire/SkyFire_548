...
bool AuthSession::_HandleLogonChallenge()
{
	...
	//SF_LOG_DEBUG("server.authserver", "[AuthChallenge] got full packet, %#04x bytes", challenge->size);
	SF_LOG_DEBUG("server.authserver", "[AuthChallenge] name(%d): '%s'", challenge->I_len, challenge->I);
	...
	if (result)
	{
		...
		SF_LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] Banned ip tries to login!", ipAddress.c_str(), port);
	}
	else
	{
		...
		if (res2)
		{
			...
			{
				SF_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '%s' is locked to IP - '%s'", _login.c_str(), fields[4].GetCString());
				SF_LOG_DEBUG("server.authserver", "[AuthChallenge] Player address is '%s'", ipAddress.c_str());
			...
			else
			{
				...
				else if (!accountCountry.empty())
				{
					...
					if (PreparedQueryResult sessionCountryQuery = LoginDatabase.Query(stmt))
					{
						...
						SF_LOG_DEBUG("server.authserver", "[AuthChallenge] Account '%s' is locked to country: '%s' Player country is '%s'", _login.c_str(),
			...
			if (!locked)
			{
				...
				if (banresult)
				{
					...
					{
						pkt << uint8(WOW_FAIL_BANNED);
						SF_LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] Banned account %s tried to login!", ipAddress.c_str(),
							port, _login.c_str());
					}
					else
					{
						pkt << uint8(WOW_FAIL_SUSPENDED);
						SF_LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] Temporarily banned account %s tried to login!",
				...
				else
				{
					...
					SF_LOG_DEBUG("network", "database authentication values: v='%s' s='%s'", databaseV.c_str(), databaseS.c_str());
					...
					SF_LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] account %s is using '%c%c%c%c' locale (%u)",
						ipAddress.c_str(), port, _login.c_str(),
...
bool AuthSession::_HandleLogonProof()
{
	...
	{
		SF_LOG_DEBUG("server.authserver", "'%s:%d' User '%s' successfully authenticated", GetRemoteIpAddress().c_str(), GetRemotePort(), _login.c_str());
	...
	else
	{
		...
		SF_LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] account %s tried to login with invalid password!",
			...
			if (PreparedQueryResult loginfail = LoginDatabase.Query(stmt))
			{
				...
				if (failed_logins >= MaxWrongPassCount)
				{
					...
					else
					{
						...
						SF_LOG_DEBUG("server.authserver", "'%s:%d' [AuthChallenge] IP got banned for '%u' seconds because account %s failed to authenticate '%u' times",
...
bool AuthSession::_HandleReconnectChallenge()
{
	...
	if (!result)
	{
		SF_LOG_ERROR("server.authserver", "'%s:%d' [ERROR] user %s tried to login and we cannot find his session key in the database.",
			GetRemoteIpAddress().c_str(), GetRemotePort(), _login.c_str());
		return false;
	}
...
bool AuthSession::_HandleReconnectProof()
{
	...
	{
		SF_LOG_ERROR("server.authserver", "'%s:%d' [ERROR] user %s tried to login, but session is invalid.", GetRemoteIpAddress().c_str(),
...
bool AuthSession::_HandleRealmList()
{
	...
	if (!result)
	{
		SF_LOG_ERROR("server.authserver", "'%s:%d' [ERROR] user %s tried to login but we cannot find him in the database.", GetRemoteIpAddress().c_str(),
			GetRemotePort(), _login.c_str());
		return false;
	}
