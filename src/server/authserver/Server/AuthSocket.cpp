...
{
    SF_LOG_INFO("server.authserver", "'%s:%d' Accepting GRUNT connection",
        socket().getRemoteAddress().c_str(), socket().getRemotePort());
}
...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    SF_LOG_INFO("server.authserver", "[AuthChallenge] name(%d): '%s'", ch->I_len, ch->I);
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (std::optional<SessionKey> K = _srp6->VerifyChallengeResponse(lp.A, lp.clientM))
    {
        ...
        SF_LOG_INFO("server.authserver", "'%s:%d' User '%s' password proof accepted",
            socket().getRemoteAddress().c_str(), socket().getRemotePort(), _login.c_str());
