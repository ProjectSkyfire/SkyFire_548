...
#pragma warning(default: 4996)
    ...
    void StoreLoginIdentityVerifier(
        ...
        SkyFire::Crypto::AuthnetSRP6::Salt const* authnetSalt = nullptr,
        SkyFire::Crypto::AuthnetSRP6::Verifier const* authnetVerifier = nullptr)
    {
        ...
        if (authnetSalt && authnetVerifier)
        {
            stmt->setBinary(6, *authnetSalt);
            stmt->setBinary(7, *authnetVerifier);
        }
        else
        {
            stmt->setNull(6);
    ...
    void StoreEmailLoginIdentityVerifier(uint32 accountId, std::string email, std::string const& password)
    {
        ...
        SkyFire::Crypto::AuthnetSRP6::RegistrationData authnet =
            SkyFire::Crypto::AuthnetSRP6::MakeRegistrationData(loginIdentity.Original, password);
        StoreLoginIdentityVerifier(accountId, LOGIN_IDENTITY_EMAIL, loginIdentity.Original, loginIdentity.Canonical,
            salt, verifier, &authnet.salt, &authnet.verifier);
    }
