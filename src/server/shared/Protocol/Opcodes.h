...
// Wire numbers and symbolic IDs are different. A zero mapping is unresolved.
...
constexpr OpcodeMetadata GetOpcodeMetadata(Opcodes id, bool server)
{
    if (server)
    {
        switch (id)
        {
#define SKYFIRE_OPCODE(symbol, number) case symbol: return {#symbol, number};
#include "ServerOpcodeCatalog.inc"
#undef SKYFIRE_OPCODE
    ...
    {
#define SKYFIRE_OPCODE(symbol, number) case symbol: return {#symbol, number};
#include "ClientOpcodeCatalog.inc"
#undef SKYFIRE_OPCODE
        default: return {nullptr, 0};
    }
}
...
inline std::string GetOpcodeNameForLogging(Opcodes id, bool isServerOpcode, uint16 opcodeNumber = 0)
{
    ...
    if (id >= NULL_OPCODE && id < UNKNOWN_OPCODE)
    {
        OpcodeMetadata const metadata = GetOpcodeMetadata(id, isServerOpcode);
        if (metadata.Name)
        {
            ss << metadata.Name;
            opcode = metadata.OpcodeNumber;
