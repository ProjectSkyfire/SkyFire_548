...
static Optional<BigNumber> GetHexFromConfig(char const* configKey, int bits)
{
    ...
    {
        SF_LOG_FATAL("server.loading", "Invalid value for '%s' - specify a hexadecimal integer of up to %d bits with no prefix.", configKey, bits);
        abort();
    }
    ...
    {
        SF_LOG_ERROR("server.loading", "Value for '%s' is out of bounds (should be an integer of up to %d bits with no prefix). Truncated to %d bits.", configKey, bits, bits);
        secret %= threshold;
    }
...
void SecretMgr::AttemptLoad(Secrets i, LogLevel errorLevel, std::unique_lock<std::mutex> const&)
{
    ...
    if (
        ...
        (oldDigest && !SkyFire::Crypto::Argon2::Verify(currentValue->AsHexStr(), *oldDigest)) // there is an old digest, and the current secret does not match it
        )
    {
        if (info.owner != THIS_SERVER_PROCESS)
        {
            if (currentValue)
                SF_LOG_ERROR("server.loading", "Invalid value for '%s' specified - this is not actually the secret being used in your auth DB.", info.configKey);
            else
                SF_LOG_ERROR("server.loading", "No value for '%s' specified - please specify the secret currently being used in your auth DB.", info.configKey);
        ...
        if (oldDigest && info.oldKey) // there is an old digest, so there might be an old secret (if possible)
        {
            ...
            if (oldSecret && !SkyFire::Crypto::Argon2::Verify(oldSecret->AsHexStr(), *oldDigest))
            {
                SF_LOG_ERROR("server.loading", "Invalid value for '%s' specified - this is not actually the secret previously used in your auth DB.", info.oldKey);
                _secrets[i].state = Secret::LOAD_FAILED;
                return;
            }
        }
        ...
        if (error)
        {
            SF_LOG_ERROR("server.loading", "Your value of '%s' changed, but we cannot transition your database to the new value:\n%s", info.configKey, error->c_str());
            _secrets[i].state = Secret::LOAD_FAILED;
            return;
        }

        SF_LOG_INFO("server.loading", "Successfully transitioned database to new '%s' value.", info.configKey);
    }
...
Optional<std::string> SecretMgr::AttemptTransition(Secrets i, Optional<BigNumber> const& newSecret, Optional<BigNumber> const& oldSecret, bool hadOldSecret) const
{
    ...
    switch (i)
    {
        case SECRET_TOTP_MASTER_KEY:
        {
            ...
            if (result) do
            {
                ...
                {
                    if (!oldSecret)
                        return "Cannot decrypt old TOTP tokens - add config key '" + std::string(secret_info[i].oldKey) + "' to authserver.conf!";

                    bool success = SkyFire::Crypto::AEDecrypt<SkyFire::Crypto::AES>(totpSecret, oldSecret->ToByteArray<SkyFire::Crypto::AES::KEY_SIZE_BYTES>());
                    if (!success)
                        return "Cannot decrypt old TOTP tokens - value of '" + std::string(secret_info[i].oldKey) + "' is incorrect for some users!";
                }

                if (newSecret)
                    SkyFire::Crypto::AEEncryptWithRandomIV<SkyFire::Crypto::AES>(totpSecret, newSecret->ToByteArray<SkyFire::Crypto::AES::KEY_SIZE_BYTES>());
    ...
    if (newSecret)
    {
        BigNumber salt;
        salt.SetRand(128);
        Optional<std::string> hash = SkyFire::Crypto::Argon2::Hash(newSecret->AsHexStr(), salt);
        if (!hash)
