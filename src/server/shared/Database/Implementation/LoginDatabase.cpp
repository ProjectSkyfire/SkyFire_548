...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(LOGIN_SEL_ACCOUNT_INFO_BY_ID, "SELECT id, session_key, last_ip, locked, expansion, mutetime, locale, recruiter, os, hasBoost FROM account WHERE id = ? AND session_key IS NOT NULL", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_ACCOUNT_INFO_BY_AUTHNET_IP, "SELECT id, session_key, last_ip, locked, expansion, mutetime, locale, recruiter, os, hasBoost FROM account WHERE last_ip = ? AND session_key IS NOT NULL ORDER BY last_login DESC, id DESC LIMIT 1", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_ACCOUNT_INFO_BY_AUTHNET_TOKEN, "SELECT a.id, s.world_session_key, s.last_ip, a.locked, a.expansion, a.mutetime, a.locale, a.recruiter, a.os, a.hasBoost FROM account_authnet_session s JOIN account a ON a.id = s.account_id WHERE s.world_account_token = ? AND s.last_ip = ? AND s.expires_at > UNIX_TIMESTAMP() ORDER BY s.updated_at DESC, s.account_id DESC LIMIT 1", CONNECTION_SYNCH);
