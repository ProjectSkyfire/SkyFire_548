...
void LoginDatabaseConnection::DoPrepareStatements()
{
    ...
    // Soft auth peer IP often differs from world connect IP (127 Soft → LAN
    // Join route, or Soft via realmListbn LAN IP → localAddress 127). Prefer
    // token+IP, then latest live token row.
    PrepareStatement(LOGIN_SEL_ACCOUNT_INFO_BY_AUTHNET_TOKEN_ANY, "SELECT a.id, s.world_session_key, s.last_ip, a.locked, a.expansion, a.mutetime, a.locale, a.recruiter, a.os, a.hasBoost FROM account_authnet_session s JOIN account a ON a.id = s.account_id WHERE s.world_account_token = ? AND s.expires_at > UNIX_TIMESTAMP() ORDER BY s.updated_at DESC, s.account_id DESC LIMIT 1", CONNECTION_SYNCH);
    PrepareStatement(LOGIN_SEL_ACCOUNT_INFO_BY_AUTHNET_LIVE, "SELECT a.id, s.world_session_key, s.last_ip, a.locked, a.expansion, a.mutetime, a.locale, a.recruiter, a.os, a.hasBoost FROM account_authnet_session s JOIN account a ON a.id = s.account_id WHERE s.expires_at > UNIX_TIMESTAMP() ORDER BY (s.last_ip = ?) DESC, s.updated_at DESC LIMIT 32", CONNECTION_SYNCH);
