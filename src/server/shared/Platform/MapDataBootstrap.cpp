...
namespace
{
    ...
    bool Run(std::vector<std::string> const& args, unsigned timeout)
    {
        ...
        bool finished = WaitForSingleObject(process.hProcess, timeout * 1000) == WAIT_OBJECT_0;
...
bool PrepareMapData(std::set<std::uint32_t>& maps, std::string& root, std::string& error)
{
    ...
    try
    {
        ...
        if (maps.empty() || maps.size() > 64 || providers.size() > 16)
            throw std::runtime_error("Configure 1..16 map providers and 1..64 maps");
