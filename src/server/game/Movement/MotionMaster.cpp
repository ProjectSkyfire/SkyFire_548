...
void MotionMaster::MoveJump(float x, float y, float z, float speedXY, float speedZ, uint32 id, uint32 arrivalSpellId)
{
    ...
    init.SetVelocity(speedXY);
    init.Launch();
    Mutate(new EffectMovementGenerator(id, arrivalSpellId), MOTION_SLOT_CONTROLLED);
}
