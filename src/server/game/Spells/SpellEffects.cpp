...
void Spell::EffectForceCast(SpellEffIndex effIndex)
{
    ...
    unitTarget->CastSpell(targets, spellInfo->Id, args);
}
...
void Spell::EffectTriggerRitualOfSummoning(SpellEffIndex effIndex)
{
    ...
    m_caster->CastSpell(NULL, spellInfo->Id, false);
}
...
void Spell::EffectEnergize(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->Id == 45051)
    {
        ...
        if (!avalibleElixirs.empty())
        {
            ...
            m_caster->CastSpell(unitTarget, Skyfire::Containers::SelectRandomContainerElement(avalibleElixirs), m_CastItem);
        }
    }
}
            ...
            switch (m_spellInfo->Id)
            {
                ...
                case 54426:
                    if (unitTarget)
                    {
                        ...
                        if (damage > 0)
                            m_caster->CastSpell(unitTarget, 28375, { SPELLVALUE_BASE_POINT0, damage });
                    }
                    return;
                ...
                case 45151:
                {
                    ...
                    float radius = m_spellInfo->Effects[effIndex].CalcRadius();
                    if (unitTarget && unitTarget->GetTypeId() == TypeID::TYPEID_PLAYER && unitTarget->GetDistance(m_caster) >= radius && !unitTarget->HasAura(46394) && unitTarget != m_caster)
                ...
                case 60243: // Blood Parrot Despawn
                    if (unitTarget->GetTypeId() == TypeID::TYPEID_UNIT && unitTarget->ToCreature()->IsSummon())
