...
void GuildMgr::LoadGuilds()
{
    ...
    SF_LOG_INFO("server.loading", "Loading guild members...");
    {
        ...
        QueryResult result = CharacterDatabase.Query("SELECT gm.guildid, gm.guid, member_rank, pnote, offnote, w.tab0, w.tab1, w.tab2, w.tab3, w.tab4, w.tab5, "
            ...
            "w.tab6, w.tab7, w.money, c.name, c.level, c.class, c.zone, c.account, c.logout_time, c.realm "
    ...
    SF_LOG_INFO("guild", "Filling bank tabs with items...");
    {
        ...
        QueryResult result = CharacterDatabase.Query("SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, enchantments, randomPropertyId, reforgeID, iit.transmogrifyId, durability, playedTime, text, "
            //   13       14     15      16         17
            "guildid, TabId, SlotId, item_guid, itemEntry FROM guild_bank_item gbi INNER JOIN item_instance ii ON gbi.item_guid = ii.guid LEFT JOIN item_instance_transmog iit ON ii.guid = iit.itemGuid");
