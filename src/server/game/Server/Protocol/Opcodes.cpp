...
#define DEFINE_OPCODE_HANDLER(opcode, opcodeNumber, status) ValidateAndSetOpcode<(opcodeNumber < NUM_OPCODE_HANDLERS), (opcode != 0)>(opcode, opcodeNumber, #opcode, status, PROCESS_INPLACE, 0);
    ...
    //These are handled in WorldSocket.cpp, never send outside
    DEFINE_OPCODE_HANDLER(SMSG_COMPRESSED_PACKET,                  0x1568, STATUS_NEVER    );  // 5.4.8 18414
    DEFINE_OPCODE_HANDLER(SMSG_MULTIPLE_PACKETS,                   0x1168, STATUS_NEVER    ); // 5.4.8 18414
