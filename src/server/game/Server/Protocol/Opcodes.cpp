...
#define DEFINE_OPCODE_HANDLER(opcode, opcodeNumber, status, processing, handler)                                      \
    ...
    DEFINE_OPCODE_HANDLER(CMSG_LFG_LFR_JOIN, 0x0000, STATUS_UNHANDLED, PROCESS_INPLACE, &WorldSession::Handle_NULL); // LFR disabled until the normal Dungeon Finder path is stable
    DEFINE_OPCODE_HANDLER(CMSG_LFG_LFR_LEAVE, 0x0000, STATUS_UNHANDLED, PROCESS_INPLACE, &WorldSession::Handle_NULL); // LFR disabled until the normal Dungeon Finder path is stable
    DEFINE_OPCODE_HANDLER(CMSG_LFG_SET_COMMENT, 0x0000, STATUS_UNHANDLED, PROCESS_INPLACE, &WorldSession::Handle_NULL); // LFG comment handler exists as HandleLfgSetCommentOpcode; opcode value still needs confirmation
...
#define DEFINE_OPCODE_HANDLER(opcode, opcodeNumber, status) ValidateAndSetOpcode<(opcodeNumber < NUM_OPCODE_HANDLERS), (opcode != 0)>(opcode, opcodeNumber, #opcode, status, PROCESS_INPLACE, 0);
    ...
    DEFINE_OPCODE_HANDLER(SMSG_DESTRUCTIBLE_BUILDING_DAMAGE, 0x14BF, STATUS_NEVER);
    ...
    DEFINE_OPCODE_HANDLER(SMSG_RAID_GROUP_ONLY, 0x0D82, STATUS_NEVER);
