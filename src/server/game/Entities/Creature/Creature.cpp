...
float Creature::GetAttackDistance(Unit const* player) const
{
    ...
    float detectionModifier = 0.0f;

    if (creaturelevel + 5 <= sWorld->getIntConfig(WorldIntConfigs::CONFIG_MAX_PLAYER_LEVEL))
    {
        detectionModifier += GetTotalAuraModifier(SPELL_AURA_MOD_DETECT_RANGE);
        detectionModifier += player->GetTotalAuraModifier(SPELL_AURA_MOD_DETECTED_RANGE);
    }

    return Skyfire::Aggro::CalculateAttackDistance(playerlevel, creaturelevel, detectionModifier, aggroRate);
}
