...
AccountOpResult AccountMgr::CreateAccount(std::string username, std::string password, std::string email = "")
{
    ...
    Skyfire::Auth::RefreshAccountLoginIdentities(accountId, username, password, salt, verifier);
...
AccountOpResult AccountMgr::DeleteAccount(uint32 accountId)
{
    ...
    Skyfire::Auth::DeleteLoginIdentities(accountId);
...
AccountOpResult AccountMgr::ChangeUsername(uint32 accountId, std::string newUsername, std::string newPassword)
{
    ...
    LoginDatabase.Execute(stmt);
    Skyfire::Auth::RefreshAccountLoginIdentities(accountId, newUsername, newPassword, salt, verifier);
...
{
    return Skyfire::Auth::ChangePassword(accountId, newPassword);
}
...
AccountOpResult AccountMgr::ChangeEmail(uint32 accountId, std::string newEmail)
{
    ...
    LoginDatabase.Execute(stmt);
    Skyfire::Auth::DeleteEmailLoginIdentities(accountId);
...
{
    return Skyfire::Auth::ConvertToEmailLogin(accountId, email, newPassword);
}

uint32 AccountMgr::GetId(std::string const& username)
{
    return Skyfire::Auth::GetId(username);
}
...
{
    return Skyfire::Auth::GetName(accountId, name);
}
...
{
    return Skyfire::Auth::CheckPassword(accountId, password);
}
