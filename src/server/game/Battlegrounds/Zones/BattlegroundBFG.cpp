...
int32 BattlegroundBFG::_GetNodeNameId(uint8 node)
{
    ...
    case BG_BFG_NODE_MINES:      return LANG_BG_BFG_NODE_MINES;
...
bool BattlegroundBFG::SetupBattleground()
{
    for (int i = 0; i < BG_BFG_DYNAMIC_NODES_COUNT; ++i)
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "BatteGroundBFG: Failed to spawn some object Battleground not created!");
            return false;
        }
    }
    ...
    {
        TC_LOG_ERROR("sql.sql", "BatteGroundBFG: Failed to spawn door object Battleground not created!");
        return false;
    }
    //buffs
    for (int i = 0; i < BG_BFG_DYNAMIC_NODES_COUNT; ++i)
    {
        if (!AddObject(BG_BFG_OBJECT_SPEEDBUFF_LIGHTHOUSE + 3 * i, Buff_Entries[0], BG_BFG_BuffPositions[i][0], BG_BFG_BuffPositions[i][1], BG_BFG_BuffPositions[i][2], BG_BFG_BuffPositions[i][3], 0, 0, std::sin(BG_BFG_BuffPositions[i][3] / 2), std::cos(BG_BFG_BuffPositions[i][3] / 2), RESPAWN_ONE_DAY)
            ...
            TC_LOG_ERROR("sql.sql", "BatteGroundBFG: Failed to spawn buff object!");
    }
...
void BattlegroundBFG::Reset()
{
    ...
    m_HonorTics = (isBGWeekend) ? BG_BFG_BFGBGWeekendHonorTicks : BG_BFG_NotBFGBGWeekendHonorTicks;
    m_ReputationTics = (isBGWeekend) ? BG_BFG_BFGBGWeekendReputationTicks : BG_BFG_NotBFGBGWeekendReputationTicks;
