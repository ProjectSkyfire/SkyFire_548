...
void BattlegroundBFG::FillInitialWorldStates(WorldStateBuilder& builder) {
    ...
    // Node icons.
    for (uint8 node = 0; node < GILNEAS_BG_DYNAMIC_NODES_COUNT; ++node)
        //data.AppendState && uint32((m_Nodes[node] == 0) ? 1 : 0) << uint32(GILNEAS_BG_OP_NODEICONS[node]);
        builder.AppendState(GILNEAS_BG_OP_NODEICONS[node], (m_Nodes[node] == 0) ? 1 : 0);

    // Node occupied states.
    for (uint8 node = 0; node < GILNEAS_BG_DYNAMIC_NODES_COUNT; ++node)
        for (uint8 i = 1; i < GILNEAS_BG_DYNAMIC_NODES_COUNT; ++i)
            //data.AppendState << uint32((m_Nodes[node] == i) ? 1 : 0) << uint32(GILNEAS_BG_OP_NODESTATES[node] + plusArray[i]);
    ...
    //data.AppendState << uint32(ally) << uint32(GILNEAS_BG_OP_OCCUPIED_BASES_ALLY);
    //data.AppendState << uint32(horde) << uint32(GILNEAS_BG_OP_OCCUPIED_BASES_HORDE);
    builder.AppendState(GILNEAS_BG_OP_OCCUPIED_BASES_ALLY, ally);
    builder.AppendState(GILNEAS_BG_OP_OCCUPIED_BASES_HORDE, horde);

    // Team scores.
    //data.AppendState << uint32(GILNEAS_BG_MAX_TEAM_SCORE) << uint32(GILNEAS_BG_OP_RESOURCES_MAX);
    //data.AppendState << uint32(GILNEAS_BG_WARNING_NEAR_VICTORY_SCORE) << uint32(GILNEAS_BG_OP_RESOURCES_WARNING);
    //data.AppendState << uint32(m_TeamScores[TEAM_ALLIANCE]) << uint32(GILNEAS_BG_OP_RESOURCES_ALLY);
    //data.AppendState << uint32(m_TeamScores[TEAM_HORDE]) << uint32(GILNEAS_BG_OP_RESOURCES_HORDE);
...
void BattlegroundBFG::_NodeOccupied(uint8 node, Team team) {
    if (!AddSpiritGuide(node, GILNEAS_BG_SpiritGuidePos[node][0], GILNEAS_BG_SpiritGuidePos[node][1], GILNEAS_BG_SpiritGuidePos[node][2], GILNEAS_BG_SpiritGuidePos[node][3], team))
        TC_LOG_ERROR("sql.sql", "Battle for Gilneas: Failed to spawn spirit guide! point: %u, team: %u, ", node, team);
...
bool BattlegroundBFG::SetupBattleground() {
    ...
    for (uint8 i = 0; i < GILNEAS_BG_DYNAMIC_NODES_COUNT; ++i) {
        if (!AddObject(GILNEAS_BG_OBJECT_BANNER_NEUTRAL + 8 * i, GILNEAS_BG_OBJECTID_NODE_BANNER_0 + i, GILNEAS_BG_NodePositions[i][0], GILNEAS_BG_NodePositions[i][1], GILNEAS_BG_NodePositions[i][2], GILNEAS_BG_NodePositions[i][3], 0, 0, sin(GILNEAS_BG_NodePositions[i][3] / 2), cos(GILNEAS_BG_NodePositions[i][3] / 2), RESPAWN_ONE_DAY)
            ...
            TC_LOG_ERROR("sql.sql", "Battle for Gilneas: Failed to spawn Banner / Banner Aura object. Battleground not created!");
            return false;
        }
    }
    ...
    if (!AddObject(GILNEAS_BG_OBJECT_GATE_A, GILNEAS_BG_OBJECTID_GATE_A, GILNEAS_BG_DoorPositions[0][0], GILNEAS_BG_DoorPositions[0][1], GILNEAS_BG_DoorPositions[0][2], GILNEAS_BG_DoorPositions[0][3], GILNEAS_BG_DoorPositions[0][4], GILNEAS_BG_DoorPositions[0][5], GILNEAS_BG_DoorPositions[0][6], GILNEAS_BG_DoorPositions[0][7], RESPAWN_IMMEDIATELY)
        ...
        TC_LOG_ERROR("sql.sql", "Battle for Gilneas: Failed to spawn Door object. Battleground not created!");
        return false;
    }
    ...
    for (uint8 i = 0; i < GILNEAS_BG_DYNAMIC_NODES_COUNT; ++i) {
        if (!AddObject(GILNEAS_BG_OBJECT_SPEEDBUFF_LIGHTHOUSE + 3 * i, Buff_Entries[0], GILNEAS_BG_BuffPositions[i][0], GILNEAS_BG_BuffPositions[i][1], GILNEAS_BG_BuffPositions[i][2], GILNEAS_BG_BuffPositions[i][3], 0, 0, sin(GILNEAS_BG_BuffPositions[i][3] / 2), cos(GILNEAS_BG_BuffPositions[i][3] / 2), RESPAWN_ONE_DAY)
            ...
            TC_LOG_ERROR("sql.sql", "Battle for Gilneas: Failed to spawn buff object!");
    }
