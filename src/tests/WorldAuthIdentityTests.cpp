...
int main()
{
    ...
    passed &= Expect(Contains(worldSocket, "usedEmailLogin, useAuthnetWorldToken, account, sessionKey"),
        "World auth should mark authnet token sessions so CONNECT_TO is limited to launcher clients.");
    passed &= Expect(Contains(characterHandler, "if (m_usesAuthnetWorldHandoff && (!m_InstanceSocket || m_InstanceSocket->IsClosed()))"),
        "Patched classic clients must enter the world on the existing socket instead of SMSG_CONNECT_TO.");
